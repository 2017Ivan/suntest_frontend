    ...
    children: [
      {
        ...
        // Tunaweza ku-pass default tab kupitia query params
        props: route => ({ tab: route.query.tab || 'login' })
      },
      // Shortcut for login - redirect to auth with login tab
      {
        path: "login",
        redirect: { name: "auth", query: { tab: "login" } }
      },
      // Shortcut for register - redirect to auth with register tab
      {
        path: "register",
