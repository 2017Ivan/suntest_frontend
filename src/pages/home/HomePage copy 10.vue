...
const transformMatch = (dbMatch) => {
  ...
  // ✅ FORMAT TIME - KWA LIVE INAONESHA TIME YA DB
  let displayTime = ''
  const timeStr = formatTimeWithAMPM(dbMatch.time)
  const dateStr = formatDate(dbMatch.date)
  const formattedDateTime = dateStr ? `${timeStr} ${dateStr}` : timeStr
  
  // Kama match iko LIVE au HALF_TIME, onyesha time ya DB
  if (dbMatch.status === 'LIVE' || dbMatch.status === 'HALF_TIME') {
    displayTime = formattedDateTime  // ← TIME KUTOKA DB
  } else {
  ...
  return {
    ...
    time: displayTime,  // ← TIME KUTOKA DB (FORMATTED)
