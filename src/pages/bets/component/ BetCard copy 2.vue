...
<template>
  <div 
    class="bg-[#0D0D0D] border border-[#2A2A2A] rounded-xl p-2 hover:border-rose-600/30 transition-all cursor-pointer hover:bg-[#1A1A1A]"
    @click="$emit('click')"
  >
    <div class="flex items-start justify-between">
      ...
      <div class="text-right flex-shrink-0 ml-4">
        ...
        <p v-if="bet.status === 'won'" class="text-green-400 font-bold text-sm mt-1.5">
          +{{ bet.actualWin?.toLocaleString() || bet.netWon?.toLocaleString() || '0' }}
        </p>
