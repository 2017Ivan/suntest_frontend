<!-- SportDetailPage.vue - Iliyorekebishwa -->
<template>
...
<div class="sticky top-0 z-20 bg-white border-b border-gray-200 shadow-sm">
  <div class="max-w-7xl mx-auto px-2 sm:px-4 py-2 sm:py-3">
    <div class="flex items-center justify-between sm:justify-center relative">
      ...
      <button 
        @click="goBack"
        class="p-1.5 sm:p-2 hover:bg-gray-100 rounded-full transition-colors duration-200 flex-shrink-0 sm:absolute sm:left-0"
      >
      ...
      <div class="text-center flex-1 sm:flex-none px-1 sm:px-0">
        <h1 class="text-[11px] sm:text-lg font-bold text-gray-800 truncate">{{ match?.homeTeam || matchData.homeTeam }} vs {{ match?.awayTeam || matchData.awayTeam }}</h1>
        <p class="text-[9px] sm:text-xs text-gray-500 truncate">{{ match?.league || matchData.league }}</p>
      </div>
...
<div class="bg-white rounded-xl shadow-sm p-3  mb-4">
  <div class="flex flex-col items-center">
    ...
    <div class="mb-2 sm:mb-4">
      <span class="px-2 sm:px-4 py-1 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-semibold bg-blue-100 text-blue-700">
    ...
    <div class="flex items-center justify-between w-full max-w-2xl gap-1 sm:gap-4">
      ...
      <div class="flex flex-col items-center flex-1">
        <div class="w-10 h-10 sm:w-16 sm:h-16 bg-gradient-to-br from-blue-100 to-blue-200 rounded-full flex items-center justify-center text-sm sm:text-2xl font-bold text-blue-700 mb-1 sm:mb-2">
          {{ getTeamInitials(match?.homeTeam || matchData.homeTeam) }}
        </div>
        <span class="text-[10px] sm:text-sm font-semibold text-gray-800 text-center line-clamp-1">{{ match?.homeTeam || matchData.homeTeam }}</span>
        <span class="text-lg sm:text-2xl font-bold text-gray-900 mt-0.5 sm:mt-1">-</span>
      </div>

      <!-- VS / Time -->
      <div class="flex flex-col items-center px-1 sm:px-4">
        <span class="text-[10px] sm:text-xs font-bold text-gray-400 uppercase">vs</span>
        <div class="w-8 h-8 sm:w-12 sm:h-12 bg-gray-100 rounded-full flex items-center justify-center mt-1 sm:mt-2">
          <span class="text-[10px] sm:text-sm font-bold text-gray-600">{{ match?.time || matchData.time || '00:00' }}</span>
        </div>
      </div>

      <!-- Away Team -->
      <div class="flex flex-col items-center flex-1">
        <div class="w-10 h-10 sm:w-16 sm:h-16 bg-gradient-to-br from-red-100 to-red-200 rounded-full flex items-center justify-center text-sm sm:text-2xl font-bold text-red-700 mb-1 sm:mb-2">
          {{ getTeamInitials(match?.awayTeam || matchData.awayTeam) }}
        </div>
        <span class="text-[10px] sm:text-sm font-semibold text-gray-800 text-center line-clamp-1">{{ match?.awayTeam || matchData.awayTeam }}</span>
        <span class="text-lg sm:text-2xl font-bold text-gray-900 mt-0.5 sm:mt-1">-</span>
      </div>
    </div>

    <!-- Match Info -->
    <div class="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-gray-100 w-full">
      <div class="flex justify-center gap-4 sm:gap-8 text-[10px] sm:text-xs text-gray-500">
      ...
      <div class="bg-gray-800  mb-4">
        <div class="space-y-2">
          ...
          <!-- 1. 1X2 -->
          <div>
            <h4 class="text-sm font-semibold text-gray-200 mb-3">1X2 | Full Time</h4>
            <div class="grid grid-cols-3 gap-3">
              <div 
                @click="handleOddsClick('1', matchData.homeOdds, '1', '1X2 | Full Time')"
                class="flex flex-row justify-between items-center cursor-pointer bg-gray-200 border border-gray-200 rounded-lg p-1 text-center hover:shadow-md transition-all"
                :class="{ ' bg-rose-600 text-gray-200': isSelectedInBetSlip('1') }"
              >
              ...
              <div 
                ...
                class="flex flex-row justify-between items-center cursor-pointer bg-gray-200 border border-gray-200 rounded-lg p-1 text-center hover:shadow-md transition-all"
                :class="{ ' bg-rose-600 text-gray-200': isSelectedInBetSlip('X') }"
              >
              ...
              <div 
                ...
                class="flex flex-row justify-between items-center cursor-pointer bg-gray-200 border border-gray-200 rounded-lg p-1 text-center hover:shadow-md transition-all"
                :class="{ ' bg-rose-600 text-gray-200': isSelectedInBetSlip('2') }"
              >
         ...
         <!-- 2. Double Chance -->
<div>
  <h4 class="text-sm font-semibold text-gray-200 mb-3">Double Chance | Full Time</h4>
  <div class="grid grid-cols-3 gap-3">
    <div 
      @click="handleOddsClick('1X', doubleChanceOdds.oneX, '1X', 'Double Chance | Full Time')"
      class="flex flex-row justify-between items-center cursor-pointer bg-gray-50 border border-gray-200 rounded-lg p-3 text-center hover:shadow-md transition-all"
    ...
    <div 
      ...
      class="flex flex-row justify-between items-center cursor-pointer bg-gray-50 border border-gray-200 rounded-lg p-1 text-center hover:shadow-md transition-all"
      ...
      <span class="text-lg font-bold text-gray-800">{{ doubleChanceOdds.oneTwo }}</span>
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              >
                <span class="text-xs text-gray-500 block">{{ key === 'yes' ? 'Yes' : 'No' }}</span>
