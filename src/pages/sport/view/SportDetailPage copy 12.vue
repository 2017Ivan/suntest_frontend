<!-- SportDetailPage.vue - SunBet Theme (Rose & Gray) -->
<template>
  <div class="min-h-screen pb-16 bg-gray-900 text-gray-100">
    <!-- Back Button & Header -->
    <div class="sticky top-0 z-20 bg-gray-900/95 backdrop-blur-md border-b border-gray-700/40 shadow-md">
      <div class="max-w-7xl mx-auto px-3 sm:px-4 py-2.5 sm:py-3">
        <div class="flex items-center justify-between relative">
          <!-- Back Button -->
          <button 
            @click="goBack"
            class="p-2 hover:bg-rose-900/40 text-gray-400 hover:text-rose-400 rounded-xl transition-colors duration-200 border border-gray-700/40"
          >
          ...
          <div class="text-center flex-1 px-2">
            <h1 class="text-sm sm:text-base font-black text-gray-100 truncate" v-if="matchData">
              {{ matchData.homeTeam }} <span class="text-rose-400 font-bold">vs</span> {{ matchData.awayTeam }}
            </h1>
            <p class="text-[10px] sm:text-xs text-gray-500 font-medium truncate" v-if="matchData">{{ matchData.league }}</p>
          </div>
    ...
    <!-- Loading State -->
    <div v-if="!matchData" class="max-w-4xl mx-auto px-3 mt-10 text-center py-12 bg-gray-800/50 rounded-2xl border border-gray-700/40">
      <div class="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-rose-500 mb-3"></div>
      <p class="text-sm font-semibold text-gray-400">Loading markets...</p>
    </div>
    ...
    <div v-else class="max-w-4xl mx-auto px-3 mt-4 space-y-4">
        ...
        <div class="flex flex-col items-center">
          ...
          <div class="flex items-center justify-between w-full max-w-xl gap-2">
            ...
            <div class="flex flex-col items-center flex-1 text-center">
              <div class="w-12 h-12 sm:w-16 sm:h-16 rounded-2xl bg-gray-800/60 border border-gray-700/60 flex items-center justify-center text-sm sm:text-2xl font-black text-rose-400 mb-2 shadow-inner">
            ...
            <div class="flex flex-col items-center px-2">
              <span class="text-xs font-black text-gray-500 bg-gray-800/60 px-2.5 py-1 rounded-lg border border-gray-700/60">VS</span>
              <span class="text-[10px] text-gray-500 mt-2 font-mono">{{ matchData.date }}</span>
            </div>
      ...
      <div class="space-y-3">
        <div 
          ...
          :key="market.key"
          class="bg-gray-800/40 rounded-2xl border border-gray-700/40 p-3 sm:p-4 shadow-sm"
        >
          <!-- Market Header Title -->
          <div class="flex justify-between items-center mb-3">
            <h3 class="text-xs sm:text-sm font-black text-gray-300 uppercase tracking-wider flex items-center gap-2">
              <span class="w-1.5 h-3 bg-rose-500 rounded-full"></span>
              {{ market.title }}
            </h3>
            <span class="text-[10px] text-gray-500 font-mono">{{ Object.keys(market.options).length }} options</span>
          </div>
...
const getOddButtonClass = (marketKey, optionKey) => {
  if (isSelected(marketKey, optionKey)) {
    return 'border-rose-500 bg-rose-500/20 shadow-lg shadow-rose-500/10 text-rose-300 font-black ring-1 ring-rose-500/50'
  }
  return 'bg-gray-800/60 border-gray-700/60 text-gray-300 hover:border-rose-500 hover:bg-gray-700/60'
}

const getLabelClass = (marketKey, optionKey) => {
  return isSelected(marketKey, optionKey) ? 'text-rose-300' : 'text-gray-300'
}
