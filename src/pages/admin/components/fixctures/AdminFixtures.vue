  ...
  <div class="space-y-4 sm:space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
      <div>
        ...
        <p class="text-rose-400 text-xs sm:text-sm">Create and manage sports fixtures for betting</p>
      </div>
      <div class="flex flex-wrap gap-2 sm:gap-3 w-full sm:w-auto">
        <button 
          @click="openCreateModal" 
          class="flex-1 sm:flex-none px-4 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-rose-500 to-rose-600 text-slate-900 rounded-xl font-bold hover:from-yellow-400 hover:to-yellow-500 transition-all shadow-lg shadow-yellow-500/20 text-sm sm:text-base"
        >
        ...
        <button 
          @click="openBulkModal" 
          class="flex-1 sm:flex-none px-4 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-teal-500 to-teal-600 text-white rounded-xl font-bold hover:from-emerald-400 hover:to-emerald-500 transition-all shadow-lg shadow-emerald-500/20 text-sm sm:text-base"
        >
          <span class="flex items-center justify-center gap-1 sm:gap-2">
            <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div v-if="matchStore.loading" class="flex justify-center py-8">
      <div class="text-rose-400 text-lg">Loading fixtures...</div>
    </div>
